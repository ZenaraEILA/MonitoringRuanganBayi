...
class ExcelExportService
{
    ...
    private static function generateSummary(Device $device, Collection $monitorings, Carbon $startDate, Carbon $endDate): array
    {
        ...
        return [
            ...
            'max_temperature' => round($monitorings->max('temperature') ?? 0, 2),
            'min_temperature' => round($monitorings->min('temperature') ?? 0, 2),
            'avg_temperature' => round($monitorings->avg('temperature') ?? 0, 2),
            'max_humidity' => round($monitorings->max('humidity') ?? 0, 2),
            'min_humidity' => round($monitorings->min('humidity') ?? 0, 2),
